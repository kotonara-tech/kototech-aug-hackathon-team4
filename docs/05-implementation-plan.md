...
### M0. 環境スパイク（担当: Native-A / Codex）

**目的**: ビルドが通らないという理由で当日詰まらないようにする。

| 項目 | 内容 |
|---|---|
| 成果物 | Kotlin Android プロジェクトの雛形。`applicationId` は `com.kotonara.farmcamera` |
| 依存 | なし |
| 完了条件 | **`ktlintCheck` / `test` / `assembleDebug` の3つ**が通る（[01 §6.1](01-overview.md)）。所要時間を記録する |
...
`gradle` は PATH に無いので wrapper を使う。

**ktlint は M0 で入れる。** 後回しにすると違反が溜まって導入できなくなる
（[03 §10](03-native.md)）。

> **要調整**: 現在手元にある雛形は `native/android/` に置かれ、ソースが
> `src/main/java/` にある。一方 [03 §4](03-native.md) は **`native/app/`** と
> **`src/main/kotlin/`** を指定している。`native/app/` は Q2 の退避で空いたので、
> 正本に合わせられる。**M1 に入る前に揃えたい。**

### M1. `domain` 層（純ロジック / テストファースト）

**目的**: 実機も GCP も要らず、**Q1 が倒れても生き残る**部分を先に固める。

[03 §9](03-native.md) の「実機なしでテストできるもの」がそのままスコープ。
置き場所は [03 §4](03-native.md) の **`domain/`**（`android.*` を import した時点で違反）。
`AppDataUploader` だけは外部に触るので `data/`。

**担当は [`native/IMPLEMENTATION_ASSIGNMENT.md`](../native/IMPLEMENTATION_ASSIGNMENT.md) に従う。**

| 担当 | 範囲 |
|---|---|
| Native-B | **M1a**: `PhotoNaming` / `CaptureScheduler` |
| Native-C | M1b: `CaptureState` / `CaptureCoordinator` / `PhotoSource`・`PhotoUploader` の interface |
| Native-D | M1c: `AppDataUploader`（OkHttp + MockWebServer） |
...
| 6 | 撮影間隔の値を決める | Q3（Open / ブロッカー） |
| 7 | 旧実装・旧ドキュメントの処遇を決める | Q2（Open / ブロッカー） |
...
### 5-1. 端末に残す JPEG の扱いが、実質的に意味を持たない

[03 §6](03-native.md) は「端末の**一時ディレクトリ**へ保存」、
[03 §11](03-native.md) は「**送信失敗時にファイルを削除しないこと**」と書いている。

この2つを合わせると次のようになる。

- 保存先は Android の `cacheDir`。**OS がストレージ逼迫時に予告なく消せる**
- 自動再送キューは作らない（[01 §5.5](01-overview.md)）
- Native はギャラリー表示も一覧取得もしない（[01 §4](01-overview.md)）

つまり **残したファイルを読む主体がどこにも存在せず、しかも OS がいつでも消せる。**
「削除しないこと」という規約が観測可能な効果を持たない。

考えられる決着は3つ。

| 案 | 内容 | 備考 |
|---|---|---|
| A | 規約どおり `cacheDir` に置いて放置 | 現状の文面。効果が無いことを承知で残す |
| B | 送信成功したら消す。失敗ぶんだけ残す | 「残す」に意味を持たせるなら最低限これ |
| C | アプリ専用の永続領域へ置き、件数上限で古いものから消す | 手動回収やログ突き合わせに使える。実装コストは C > B > A |

**私からの推奨は B。** 読む主体がいない以上 C の永続化はコストに見合わず、
A は規約が空文になる。B なら「失敗したぶんだけが端末に残っている」が観測可能な事実になる。

> 経緯: 旧実装ではこれを C の方向（`getApplicationDocumentsDirectory` + 件数上限）で
> 実装し、PR #39 として出してある。**新方針では過剰**と判断して取り下げを提案する（§6）。
...
## 6. 既存の issue / PR の処遇（提案）

`native/app`（Flutter）を前提にした issue が残っている。**Q2 が決まるまで実際には触らない。**

| # | 件名 | 提案 |
|---|---|---|
| **39**(PR) | 写真をアプリ専用領域に保存し上限超過ぶんを削除 | **取り下げ**。5-1 案 C に相当し、新方針では過剰 |
| 32 | 同上の issue | クローズ。5-1 の決着を新 issue で立て直す |
...
## 8. 完了の定義（[01 §6.1](01-overview.md)）

**CI は回さない。** 壊れたコードを止めるのは実装者が最後に回すコマンドだけ。
...
**3つすべてを実際に実行し、出力を確認してから「できた」と言う。**
PowerShell 5.1 では `&&` が使えないので `;` か `if ($?) { ... }` を使う。

ブランチは **`develop` から切り、`develop` へ PR** する（[CLAUDE.md §6](../CLAUDE.md)）。

---

## 9. 私の担当分の着手順（確定）

**担当は Native-B（M1a: `PhotoNaming` / `CaptureScheduler`）。**
M1b・M1c の依存元なので、ここを最初に通す。

1. M0 の雛形の置き場所を [03 §4](03-native.md) に揃える（`native/app/` + `src/main/kotlin/`）
2. **M1a** をテストファーストで実装する。Q1 が倒れても残る
3. 人間側の §4 が揃い次第、Native-A + D の **M2** に合流し、02 §7 の検証を実行する
4. 検証が通ってから M3 → M4 → M5
