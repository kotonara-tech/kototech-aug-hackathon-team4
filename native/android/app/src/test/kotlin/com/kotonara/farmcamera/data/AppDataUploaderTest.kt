...
class AppDataUploaderTest {
    ...
    @Test
    fun `AppData の multipart URL と Bearer と親フォルダを送る`() =
        runBlocking {
            ...
            assertTrue(lookupRequest.path!!.contains("spaces=appDataFolder"))
    ...
    @Test
    fun `201 response returns the Drive file ID`() =
        runBlocking {
            server.enqueue(MockResponse().setResponseCode(200).setBody("""{"files":[]}"""))
            server.enqueue(MockResponse().setResponseCode(201).setBody("""{"id":"file-created"}"""))

            val result = uploader().upload("CAM001_20260822_063000.jpg", byteArrayOf())

            assertEquals("file-created", result.getOrThrow())
        }

    @Test
    fun `ファイル ID がない成功レスポンスは失敗として返す`() =
        runBlocking {
            server.enqueue(MockResponse().setResponseCode(200).setBody("""{"files":[]}"""))
    ...
    @Test
    fun `アクセストークン取得に失敗したら HTTP リクエストを送らない`() =
        runBlocking {
            val uploader =
                AppDataUploader(
                    client = OkHttpClient(),
    ...
    @Test
    fun `既定の Drive URL を使う場合でもトークン失敗を返す`() =
        runBlocking {
            val uploader =
                AppDataUploader(
                    client = OkHttpClient(),
    ...
    @Test
    fun `Drive に同名ファイルがある場合はアップロードしない`() =
        runBlocking {
            server.enqueue(MockResponse().setResponseCode(200).setBody("""{"files":[{"id":"existing-file"}]}"""))

            val result = uploader().upload("CAM001_20260822_063000.jpg", byteArrayOf())
            val request = server.takeRequest()

            assertEquals("existing-file", result.getOrThrow())
